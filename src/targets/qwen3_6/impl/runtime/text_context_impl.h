...
const Tensor& TextContext::rank_linear_state_source_slots(int rank) const {
    if (rank == 0) {
        if (active_linear_state_source_slots_ == nullptr) {
            throw std::logic_error("Linear Attention source state slots are unbound");
        }
        return *active_linear_state_source_slots_;
    }
    if (peer_linear_state_source_slots_ == nullptr) {
        throw std::logic_error("tensor-parallel peer source state slots are unbound");
    }
    return *peer_linear_state_source_slots_;
}
...
const Tensor& TextContext::rank_linear_state_destination_slots(int rank) const {
    if (rank == 0) {
        if (active_linear_state_destination_slots_ == nullptr) {
            throw std::logic_error("Linear Attention destination state slots are unbound");
        }
        return *active_linear_state_destination_slots_;
    }
    if (peer_linear_state_destination_slots_ == nullptr) {
        throw std::logic_error("tensor-parallel peer destination state slots are unbound");
    }
    return *peer_linear_state_destination_slots_;
}
...
void TextContext::attn_mix_tp2(const FullLayerW& w0, const FullLayerW& w1, std::array<Tensor, 2>& x,
    ...
    for_each_rank(execution, [&](int rank) {
        ...
        if (active_sequence_batch_ != 0) {
            ...
            ops::causal_softmax_attention(
                q_batch, k_batch, v_batch, position_batch, rank_valid_columns(rank),
                rank_kv_table_rows(rank), {kCfg.head_dim, kShardQHeads, kShardKvHeads},
                kAttnScale, pages.batch_layer_view(fidx), *active_causal_attention_envelope_,
                *ws[r], a_batch, s);
        } else {
            ops::causal_softmax_attention(
                qn[r], kn[r], v[r], cache, Tensor{}, rank_kv_table_rows(rank),
                {kCfg.head_dim, kShardQHeads, kShardKvHeads}, kAttnScale,
                pages.batch_layer_view(fidx), *active_causal_attention_envelope_, *ws[r], a[r], s);
        }
...
void TextContext::gdn_mix_tp2(const GdnLayerW& w0, const GdnLayerW& w1, std::array<Tensor, 2>& x,
    ...
    if (ph == Phase::Verify) {
        ...
        std::array<Tensor, 2> source_slots;
        std::array<Tensor, 2> destination_slots;
        for (std::size_t r = 0; r < 2; ++r) {
            ...
            conv_states[r] =
                state_for(rank).layer_view(static_cast<std::uint32_t>(gidx)).conv;
        ...
        if (gdn_state_action_ == GdnStateAction::RecordForReplay) {
            ...
            Variant::gdn_input_projection_record(projection_input, {w0.projection, w1.projection},
                                                 conv_weight, conv_states, valid, source_slots,
            ...
            Variant::gdn_input_projection_snapshot(
                ...
                source_slots, destination_slots, query_output, key_output, value_output,
                gate_output, ph, ws, execution);
        }
    } else {
        ...
        for_each_rank(execution, [&](int rank) {
            const auto r      = static_cast<std::size_t>(rank);
            cudaStream_t s    = stream_for(rank);
            Tensor conv_state_in = state_for(rank).conv_slot(
                static_cast<std::uint32_t>(gidx), linear_state_source_slot_);
            Tensor conv_state_out = state_for(rank).conv_slot(
                static_cast<std::uint32_t>(gidx), linear_state_destination_slot_);
            ops::causal_conv1d_silu_split(qkv[r], *w[r]->conv1d, conv_state_in, conv_state_out,
                                          qc[r], kc[r], vc[r], s);
        });
    }
    ...
    for_each_rank(execution, [&](int rank) {
        ...
        if (ph == Phase::Verify) {
            Tensor recurrent_states =
                state_for(rank).layer_view(static_cast<std::uint32_t>(gidx)).recurrent;
            const std::int32_t width = active_sequence_width_;
            Tensor q_batch =
            ...
            const Tensor source_slots = rank_linear_state_source_slots(rank);
            if (gdn_state_action_ == GdnStateAction::RecordForReplay) {
                GdnReplayRecordLayer records =
                    replay_records_for(rank)->layer(gidx, active_sequence_batch_);
                ops::gated_delta_net_replay_record(q_batch, k_batch, v_batch, g_batch, beta_batch,
                                                   kGdnScale, recurrent_states, valid, source_slots,
                                                   records.key, records.value, records.gate,
                                                   out_batch, s);
            } else {
                ops::gated_delta_net_batch_update(
                    q_batch, k_batch, v_batch, g_batch, beta_batch, kGdnScale,
                    /*normalize_qk=*/true, recurrent_states, source_slots,
                    rank_linear_state_destination_slots(rank), out_batch, s);
            }
        } else {
            Tensor recurrent_state_in = state_for(rank).recurrent_slot(
                static_cast<std::uint32_t>(gidx), linear_state_source_slot_);
            Tensor recurrent_state_out = state_for(rank).recurrent_slot(
                static_cast<std::uint32_t>(gidx), linear_state_destination_slot_);
...
PrefillChunkResult TextContext::prefill_impl_tp2(std::span<const int> ids,
    ...
    const std::int64_t checkpoint_abs = prefill_split_frontier_;
...
void TextContext::ordinary_decode_batch_tp2(const Tensor& ids, const Tensor& cache_positions,
    ...
    const Tensor peer_state_sources = peer.state_source_slots.slice(0, 0, batch);
    const Tensor peer_state_destinations = peer.state_destination_slots.slice(0, 0, batch);
    ...
    tp_->work->reset();
    {
        ...
        ScopedValue<const Tensor*> peer_source_binding(peer_linear_state_source_slots_,
                                                       &peer_state_sources);
        ScopedValue<const Tensor*> peer_destination_binding(
            peer_linear_state_destination_slots_, &peer_state_destinations);
...
void TextContext::mtp_forward_tail_tp2(std::array<Tensor, 2>& x, const std::array<Tensor, 2>& ah,
    ...
    for_each_rank(execution, [&](int rank) {
        ...
        if (active_sequence_batch_ != 0) {
            ...
            ops::causal_softmax_attention(
                q_batch, k_batch, v_batch, position_batch, rank_valid_columns(rank),
                rank_backend_kv_table_rows(rank), {kCfg.head_dim, kShardQHeads, kShardKvHeads},
                kAttnScale, pages.batch_layer_view(0), envelope, *ws[r], a_batch, s);
        } else {
            ops::causal_softmax_attention(
                qn[r], kn[r], v[r], positions[r], Tensor{}, io_for(rank).backend_kv_table_row,
                {kCfg.head_dim, kShardQHeads, kShardKvHeads}, kAttnScale,
...
void TextContext::target_verify_batch(const std::array<Tensor, 2>& ids,
    ...
    tp_->work->reset();
    {
        ...
        if (feature_sink != nullptr) { feature_sink->begin(x[0]); }
        run_layers_tp2(x, Phase::Verify, staging, feature_sink);
        if (feature_sink != nullptr) {
            feature_sink->capture_positions(cache_positions[0], ctx_.stream);
        }
...
void TextContext::mtp_prefill_chunk_tp2(const Tensor& ids, const std::array<Tensor, 2>& hidden,
...

    {
        ...
        for_each_rank(execution, [&](int rank) {
            ...
            ops::kv_cache_append(kn, v, positions[r], pages.layer_view(0), s);
            if (final_chunk) {
                const std::size_t column_bytes =
                    static_cast<std::size_t>(kCfg.hidden) * dtype_size(DType::BF16);
    ...
    for_each_rank(execution, [&](int rank) {
        ...
        ops::causal_softmax_attention_cached(
            qn, last_position, {kCfg.head_dim, kShardQHeads, kShardKvHeads}, kAttnScale,
            pages.layer_view(0), envelope, *ws[r], a[r], s);
